...
def import_jobs(api_base: str, jobs_dir: Path, manifest_path: Path) -> dict[str, Any]:
    ...
    for path in job_files:
        ...
        if isinstance(existing, dict) and existing.get("job_id"):
            skipped += 1
            print(f"skip  {key}: already mapped to {existing['job_id']}")
            continue
...
def _report_markdown(report: dict[str, Any]) -> str:
    ...
    if not isinstance(ranking, list):
        raise ValueError("report ranking must be a list")
    ...
    for index, item in enumerate(ranking, start=1):
        ...
        score = "—" if score_value is None else f"{score_value}%"
...
def evaluate(
    ...
    for label in labels:
        key_value = label["job_file"]
        if not isinstance(key_value, str):
            raise ValueError("label job_file must be text")
        key = key_value
        entry = jobs.get(key)
        ...
        if not isinstance(relevance, int) or not isinstance(blocker_real, bool):
            raise ValueError(f"invalid human label for {key}")
        if reason is not None and not isinstance(reason, str):
            raise ValueError(f"invalid reason for {key}")
        ...
        print(
            f"eval   {key}: score={display_score} coverage={coverage}% human={relevance}"
        )
