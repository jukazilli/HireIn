...
class CandidateProfile(Base):
    ...
    preference: Mapped[CareerPreference | None] = relationship(
...
class CandidateExperience(Base):
    ...
    facts: Mapped[list[CandidateFact]] = relationship(
        back_populates="experience", cascade="all, delete-orphan", passive_deletes=True
    )
...
class CandidateSkill(Base):
    ...
    __table_args__ = (
        UniqueConstraint("profile_id", "normalized_name", name="uq_candidate_skills_profile_name"),
    )
...
class CandidateFact(Base):
    ...
    experience_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("candidate_experiences.id", ondelete="CASCADE"), index=True
    )
