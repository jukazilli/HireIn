...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint("char_length(country_code) = 2", name="ck_candidate_profiles_country_code"),
    )
    ...
    op.create_table(
        ...
        sa.Column("desired_titles", postgresql.JSONB(astext_type=sa.Text()), nullable=False, server_default=sa.text("'[]'::jsonb")),
        sa.Column("desired_areas", postgresql.JSONB(astext_type=sa.Text()), nullable=False, server_default=sa.text("'[]'::jsonb")),
        sa.Column("seniority_levels", postgresql.JSONB(astext_type=sa.Text()), nullable=False, server_default=sa.text("'[]'::jsonb")),
        sa.Column("work_models", postgresql.JSONB(astext_type=sa.Text()), nullable=False, server_default=sa.text("'[]'::jsonb")),
        sa.Column("contract_types", postgresql.JSONB(astext_type=sa.Text()), nullable=False, server_default=sa.text("'[]'::jsonb")),
        sa.Column("target_locations", postgresql.JSONB(astext_type=sa.Text()), nullable=False, server_default=sa.text("'[]'::jsonb")),
        ...
        sa.CheckConstraint("salary_min IS NULL OR salary_max IS NULL OR salary_min <= salary_max", name="ck_career_preferences_salary_range"),
    )
    ...
    op.create_table(
        ...
        *_provenance_columns(),
        sa.PrimaryKeyConstraint("id"),
        sa.CheckConstraint("end_date IS NULL OR end_date >= start_date", name="ck_candidate_experiences_dates"),
        sa.CheckConstraint("NOT is_current OR end_date IS NULL", name="ck_candidate_experiences_current_end_date"),
    ...
    op.create_table(
        ...
        *_provenance_columns(),
        sa.PrimaryKeyConstraint("id"),
        sa.CheckConstraint("status IN ('IN_PROGRESS','COMPLETED','PAUSED','DROPPED')", name="ck_candidate_education_status"),
        sa.CheckConstraint("start_date IS NULL OR end_date IS NULL OR end_date >= start_date", name="ck_candidate_education_dates"),
    ...
    op.create_table(
        ...
        *_provenance_columns(),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("profile_id", "normalized_name", name="uq_candidate_skills_profile_name"),
        sa.CheckConstraint("level IS NULL OR level IN ('BEGINNER','INTERMEDIATE','ADVANCED','EXPERT')", name="ck_candidate_skills_level"),
        sa.CheckConstraint("years_experience IS NULL OR years_experience >= 0", name="ck_candidate_skills_years"),
    ...
    op.create_table(
        ...
        *_provenance_columns(),
        sa.PrimaryKeyConstraint("id"),
        sa.CheckConstraint("issued_date IS NULL OR expires_date IS NULL OR expires_date >= issued_date", name="ck_candidate_certifications_dates"),
        *_provenance_constraints("candidate_certifications"),
    )
    op.create_index("ix_candidate_certifications_profile_id", "candidate_certifications", ["profile_id"])
    ...
    op.create_table(
        ...
        *_provenance_columns(),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("profile_id", "normalized_name", name="uq_candidate_languages_profile_name"),
        sa.CheckConstraint("proficiency IN ('BASIC','ELEMENTARY','INTERMEDIATE','UPPER_INTERMEDIATE','ADVANCED','FLUENT','NATIVE')", name="ck_candidate_languages_proficiency"),
    ...
    op.create_table(
        ...
        sa.Column("id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column("profile_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("candidate_profiles.id", ondelete="CASCADE"), nullable=False),
        sa.Column("experience_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("candidate_experiences.id", ondelete="CASCADE"), nullable=True),
        sa.Column("kind", sa.String(length=32), nullable=False),
        sa.Column("value", sa.Text(), nullable=False),
        *_provenance_columns(),
        sa.PrimaryKeyConstraint("id"),
        sa.CheckConstraint("kind IN ('RESPONSIBILITY','ACHIEVEMENT','TOOL','DOMAIN','PROJECT','OTHER')", name="ck_candidate_facts_kind"),
