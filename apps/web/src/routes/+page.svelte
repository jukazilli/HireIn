...
<main>
  ...
  <form onsubmit={save}>
    ...
    </div><div class="choices">{#each [{value:'REMOTE',label:'Remoto'},{value:'HYBRID',label:'Híbrido'},{value:'ONSITE',label:'Presencial'}] as option}<label><input type="checkbox" checked={form.work_models.includes(option.value)} onchange={() => form.work_models = toggle(form.work_models, option.value)} />{option.label}</label>{/each}</div>
