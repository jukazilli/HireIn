# HireIn Resume Rewriter — Prompt v1
...
The output must conform exactly to the provided `ResumeRewriteCandidate` JSON schema.
...
Return only data that matches the `ResumeRewriteCandidate` schema supplied by the adapter.

Do not add markdown, prose outside the schema, explanations or hidden assumptions.

The adapter supplies these runtime fields and they must be preserved exactly:

- `provider`;
- `model`;
- `prompt_version = "v1"`.

The benchmark must use this same prompt across providers before any provider-specific tuning is considered.
